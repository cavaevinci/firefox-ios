...
class SettingAppearanceTest: BaseTestCase {
    ...
    func checkTrackingProtectionSettingsPage() {
    ...
    func testCheckSetting() {
        ...
        checkTrackingProtectionSettingsPage()
    }
    ...
    func testOpenInSafari() {
        ...
        // Failed to launch org.mozilla.ios.Focus: The operation couldn’t be completed.
    ...
    func testSafariIntegration() {
        ...
        iOSSettings.activate()
        waitForExistence(iOSSettings.cells["Safari"], timeout: 10)
        iOSSettings.cells["Safari"].tap()
        iOSSettings.cells["AutoFill"].swipeUp()
        if #available(iOS 15.0, *) {
            iOSSettings.cells.staticTexts["Extensions"].tap()
        } else {
            iOSSettings.cells.staticTexts["CONTENT_BLOCKERS"].tap()
        }
        iOSSettings.tables.cells.staticTexts["Firefox Focus"].tap()
        iOSSettings.tables.cells.switches.element(boundBy: 0).tap()
        iOSSettings.terminate()
